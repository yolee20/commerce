...
import com.itstar.commons.model._
import com.itstar.commons.utils._
...
object UserVisitSessionAnalyze {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val filteredSessionid2AggrInfoRDD = this.filterSessionAndAggrStat(sessionid2AggrInfoRDD, taskParam, sessionAggrStatAccumulator)
    ...
    val sessionid2detailRDD = this.getSessionid2detailRDD(filteredSessionid2AggrInfoRDD, sessionid2actionRDD)

    // 对数据进行内存缓存
    sessionid2detailRDD.persist(StorageLevel.MEMORY_ONLY)
    // 业务需求一：统计各个范围的session占比，并写入MySQL
    this.calculateAndPersistAggrStat(spark, sessionAggrStatAccumulator.value, taskUUID)

    // 业务需求二：随机均匀获取Session，之所以业务功能二先计算，是为了通过Action操作触发所有转换操作。
    this.randomExtractSession(spark, taskUUID, filteredSessionid2AggrInfoRDD, sessionid2detailRDD)

    // 业务需求三：获取top10热门品类
    // 返回排名前十的品类是为了在业务功能四中进行使用
    val top10CategoryList = this.getTop10Category(spark, taskUUID, sessionid2detailRDD)

    // 业务需求四：获取top10热门品类的活跃session
    this.getTop10Session(spark, taskUUID, top10CategoryList, sessionid2detailRDD)
  ...
  /**
    ...
    * @return
    */
  def getSessionid2detailRDD(sessionid2aggrInfoRDD: RDD[(String, String)], sessionid2actionRDD: RDD[(String, UserVisitAction)])
                             : RDD[(String, UserVisitAction)] = {
  ...
  /**
    ...
    * @param spark
    * @param taskUUID
    * @param sessionid2AggrInfoRDD  RDD[(sessionid, fullAggrInfo)]
    * @param sessionid2actionRDD  RDD[(sessionid,UserVisitAction)]
    */
  def randomExtractSession(spark: SparkSession,
    ...
    val time2sessionidRDD = sessionid2AggrInfoRDD.map {
    ...
    for ((dateHour, count) <- countMap) {
      ...
      dateHourCountMap.get(date) match {
        ...
        case None =>
          ...
          dateHourCountMap(date) += (hour -> count)//<HH,count>
        // 对应日期的数据存在，则更新
    ...
    // 第三步：遍历每天每小时的session，然后根据随机索引进行抽取,我们用flatMap算子，
    // 遍历所有的<dateHour,(session aggrInfo)>格式的数据
  ...
  def getTop10Category(spark: SparkSession,
                       taskid: String,
                       sessionid2detailRDD: RDD[(String, UserVisitAction)]): Array[(CategorySortKey, String)] = {

    // 第一步：获取每一个Sessionid 点击、下单、支付的数量categoryidRDD（long,long）
    ...
    // 对重复的categoryid进行去重   categoryidRDD（long,long）
    ...
    // 第三步：join各品类与它的点击、下单和支付的次数，返回结果(categoryid, value)  value是不同categoryid点击、下单、支付的计数
    ...
    // 第四步：自定义二次排序。创建用于二次排序的联合key，将数据映射成<CategorySortKey,info>格式的RDD，然后进行二次排序（降序）
    // 二次排序的联合key —— (CategorySortKey(clickCount, orderCount, payCount), line)
  ...
  /**
    * 业务功能三：4、连接品类RDD与数据RDD(点击、下单、支付）
    *
    * @param categoryidRDD
  ...
  def joinCategoryAndData(categoryidRDD: RDD[(Long, Long)],
    ...
    val payJoinRDD = orderJoinRDD.leftOuterJoin(payCategoryId2CountRDD).map { case (categoryid, (pvalue, optionValue)) =>
      val payCount = if (optionValue.isDefined) optionValue.get else 0L
      val value = pvalue + "|" + Constants.FIELD_PAY_COUNT + "=" + payCount
      (categoryid, value)
    }
  ...
  /**
    ...
    * @param taskid
    */
  def getTop10Session(spark: SparkSession, taskid: String,
